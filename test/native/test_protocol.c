...
static int test_parse_ping(void) {
    ...
    if (result != 0) FAIL("parse failed");
    if (req.id != 1) FAIL("wrong id");
    if (req.method != RPC_METHOD_PING) FAIL("wrong method");
...
static int test_parse_frost_sign(void) {
    ...
    int result = protocol_parse_request(json, &req);
    if (result != 0) FAIL("parse failed");
    if (req.id != 42) FAIL("wrong id");
    if (req.method != RPC_METHOD_FROST_SIGN) FAIL("wrong method");
    if (strcmp(req.group, "npub1abc") != 0) FAIL("wrong group");
    if (strcmp(req.message, "deadbeef") != 0) FAIL("wrong message");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_parse_import_share(void) {
    ...
    int result = protocol_parse_request(json, &req);
    if (result != 0) FAIL("parse failed");
    if (req.id != 5) FAIL("wrong id");
    if (req.method != RPC_METHOD_IMPORT_SHARE) FAIL("wrong method");
    if (strcmp(req.group, "npub1xyz") != 0) FAIL("wrong group");
    if (strcmp(req.share, "aabbcc") != 0) FAIL("wrong share");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_parse_invalid_json(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("not json", &req) != PROTOCOL_ERR_PARSE) FAIL("should fail");
    if (protocol_parse_request("{incomplete", &req) != PROTOCOL_ERR_PARSE) FAIL("incomplete should fail");
    if (protocol_parse_request("", &req) != PROTOCOL_ERR_PARSE) FAIL("empty should fail");
    if (protocol_parse_request("null", &req) != PROTOCOL_ERR_PARSE) FAIL("null should fail");
    if (protocol_parse_request("[]", &req) != PROTOCOL_ERR_PARSE) FAIL("array should fail");
    PASS();
    return 0;
}
...
static int test_parse_missing_id(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"method\":\"ping\"}", &req) != PROTOCOL_ERR_PARSE) FAIL("should fail");
    PASS();
    return 0;
}
...
static int test_parse_missing_method(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1}", &req) != PROTOCOL_ERR_PARSE) FAIL("should fail");
    PASS();
    return 0;
}
...
static int test_parse_id_wrong_type(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":\"str\",\"method\":\"ping\"}", &req) != PROTOCOL_ERR_PARSE) FAIL("string id should fail");
    if (protocol_parse_request("{\"id\":null,\"method\":\"ping\"}", &req) != PROTOCOL_ERR_PARSE) FAIL("null id should fail");
    PASS();
    return 0;
}
...
static int test_parse_method_wrong_type(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":123}", &req) != PROTOCOL_ERR_PARSE) FAIL("number method should fail");
    if (protocol_parse_request("{\"id\":1,\"method\":null}", &req) != PROTOCOL_ERR_PARSE) FAIL("null method should fail");
    PASS();
    return 0;
}
...
static int test_format_success(void) {
    ...
    if (strstr(buf, "\"id\":1") == NULL) FAIL("missing id");
    if (strstr(buf, "\"result\"") == NULL) FAIL("missing result");
    if (strstr(buf, "\"pong\":true") == NULL) FAIL("missing pong");
    PASS();
    return 0;
}
...
static int test_format_error(void) {
    ...
    int len = protocol_format_response(&resp, buf, sizeof(buf));
    if (len <= 0) FAIL("format failed");
    if (strstr(buf, "\"id\":2") == NULL) FAIL("missing id");
...
static int test_all_methods(void) {
    ...
    struct { const char *json; rpc_method_t expected; } cases[] = {
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        if (result != 0) FAIL("parse failed");
        if (req.method != cases[i].expected) FAIL("wrong method");
        protocol_free_request(&req);
    }
    PASS();
    return 0;
...
static int test_threshold_boundary(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"threshold\":0}}", &req) != 0) FAIL("0 should work");
    protocol_free_request(&req);
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"threshold\":16}}", &req) != 0) FAIL("16 should work");
    protocol_free_request(&req);
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"threshold\":17}}", &req) != PROTOCOL_ERR_PARAMS) FAIL("17 should fail");
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"threshold\":-1}}", &req) != PROTOCOL_ERR_PARAMS) FAIL("-1 should fail");
    PASS();
    return 0;
}
...
static int test_participant_count_boundary(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"participant_count\":16}}", &req) != 0) FAIL("16 should work");
    protocol_free_request(&req);
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"participant_count\":17}}", &req) != PROTOCOL_ERR_PARAMS) FAIL("17 should fail");
    PASS();
    return 0;
}
...
static int test_our_index_boundary(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"our_index\":16}}", &req) != 0) FAIL("16 should work");
    protocol_free_request(&req);
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_init\",\"params\":{\"our_index\":17}}", &req) != PROTOCOL_ERR_PARAMS) FAIL("17 should fail");
    PASS();
    return 0;
}
...
static int test_peer_index_boundary(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_round1_peer\",\"params\":{\"peer_index\":16}}", &req) != 0) FAIL("16 should work");
    protocol_free_request(&req);
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_round1_peer\",\"params\":{\"peer_index\":17}}", &req) != PROTOCOL_ERR_PARAMS) FAIL("17 should fail");
    PASS();
    return 0;
}
...
static int test_psbt_too_long(void) {
    ...
    snprintf(json, sizeof(json), "{\"id\":1,\"method\":\"bitcoin_sign\",\"params\":{\"psbt\":\"%s\"}}", psbt);
    rpc_request_t req;
    if (protocol_parse_request(json, &req) != PROTOCOL_ERR_PARAMS) FAIL("should fail");
    PASS();
    return 0;
}
...
static int test_psbt_allocation(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"bitcoin_sign\",\"params\":{\"psbt\":\"cHNidP8B\"}}", &req) != 0) FAIL("parse failed");
    if (req.psbt == NULL) FAIL("psbt should be allocated");
    if (strcmp(req.psbt, "cHNidP8B") != 0) FAIL("wrong psbt value");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_dkg_params(void) {
    TEST("dkg params");
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"dkg_round1\",\"params\":{\"group\":\"test\",\"dkg_data\":\"abc123\"}}", &req) != 0) FAIL("parse failed");
    if (strcmp(req.group, "test") != 0) FAIL("wrong group");
    if (strcmp(req.dkg_data, "abc123") != 0) FAIL("wrong dkg_data");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_session_id_param(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"frost_commit\",\"params\":{\"session_id\":\"deadbeef\"}}", &req) != 0) FAIL("parse failed");
    if (strcmp(req.session_id, "deadbeef") != 0) FAIL("wrong session_id");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_commitments_param(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"frost_sign\",\"params\":{\"commitments\":\"aabbccdd\"}}", &req) != 0) FAIL("parse failed");
    if (strcmp(req.commitments, "aabbccdd") != 0) FAIL("wrong commitments");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_policy_bundle_param(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"policy_update\",\"params\":{\"bundle\":\"data\"}}", &req) != 0) FAIL("parse failed");
    if (strcmp(req.policy_bundle, "data") != 0) FAIL("wrong bundle");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_input_idx_param(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"bitcoin_sign\",\"params\":{\"input_idx\":5}}", &req) != 0) FAIL("parse failed");
    if (req.input_idx != 5) FAIL("wrong input_idx");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_format_buffer_too_small(void) {
    ...
    if (len != -1) FAIL("should fail with small buffer");
    PASS();
    return 0;
}
...
static int test_empty_params(void) {
    ...
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"ping\",\"params\":{}}", &req) != 0) FAIL("parse failed");
    if (req.method != RPC_METHOD_PING) FAIL("wrong method");
    protocol_free_request(&req);
    PASS();
    return 0;
...
static int test_null_params(void) {
    TEST("null params");
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":1,\"method\":\"ping\",\"params\":null}", &req) != 0) FAIL("parse failed");
    protocol_free_request(&req);
    PASS();
    return 0;
}
...
static int test_negative_id(void) {
    TEST("negative id");
    rpc_request_t req;
    if (protocol_parse_request("{\"id\":-5,\"method\":\"ping\"}", &req) != 0) FAIL("parse failed");
    if (req.id != -5) FAIL("wrong id");
    protocol_free_request(&req);
    PASS();
    return 0;
