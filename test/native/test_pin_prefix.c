...
static int test_different_prefix_different_words(void) {
    ...
    if (pin_prefix_derive_words(&prefix1, TEST_SECRET, sizeof(TEST_SECRET), &word1_a, &word2_a) !=
        0)
        FAIL("first derive failed");
    if (pin_prefix_derive_words(&prefix2, TEST_SECRET, sizeof(TEST_SECRET), &word1_b, &word2_b) !=
        0)
        FAIL("second derive failed");
