...
static uint32_t test_now_ms(void) { return mock_time_ms; }
...
static int test_session_id_valid(void) {
    ...
    if (!frost_is_session_id_valid(valid_id)) FAIL("valid ID rejected");

    uint8_t zero_id[32] = {0};
    if (frost_is_session_id_valid(zero_id)) FAIL("zero ID accepted");

    uint8_t all_ones[32];
    memset(all_ones, 0xff, 32);
    if (frost_is_session_id_valid(all_ones)) FAIL("all-ones ID accepted");

    uint8_t almost_zero[32] = {0};
    almost_zero[15] = 0x01;
    if (!frost_is_session_id_valid(almost_zero)) FAIL("almost-zero rejected");

    uint8_t almost_ones[32];
    memset(almost_ones, 0xff, 32);
    almost_ones[0] = 0xfe;
    if (!frost_is_session_id_valid(almost_ones)) FAIL("almost-ones rejected");

    if (frost_is_session_id_valid(NULL)) FAIL("NULL accepted");
...
static int test_parse_commitment(void) {
    ...
    if (frost_parse_commitment(valid_hex, out, &index) != 0) FAIL("valid hex failed");
    if (index != 0x0201) FAIL("wrong index parsed");

    char short_hex[COMMITMENT_HEX_LEN];
    memset(short_hex, '0', COMMITMENT_HEX_LEN - 1);
    short_hex[COMMITMENT_HEX_LEN - 1] = '\0';
    if (frost_parse_commitment(short_hex, out, &index) != FROST_CORE_ERR_PARSE) FAIL("short hex accepted");

    if (frost_parse_commitment(NULL, out, &index) != FROST_CORE_ERR_PARSE) FAIL("NULL hex accepted");
    if (frost_parse_commitment(valid_hex, NULL, &index) != FROST_CORE_ERR_PARSE) FAIL("NULL out accepted");
...
static int test_parse_commitments_empty(void) {
    ...
    if (result != 0) FAIL("empty string should return 0");
    if (session.commitment_count != 0) FAIL("count should be 0");
...
static int test_parse_commitments_single(void) {
    ...
    if (result != 1) FAIL("should parse 1 commitment");
    if (session.commitment_count != 1) FAIL("count should be 1");
    if (session.commitment_indices[0] != 3) FAIL("wrong index");
...
static int test_parse_commitments_multiple(void) {
    ...
    int result = frost_parse_commitments(hex, &session);
    if (result != 2) FAIL("should parse 2 commitments");
    if (session.commitment_count != 2) FAIL("count should be 2");
    if (session.commitment_indices[0] != 1) FAIL("wrong first index");
    if (session.commitment_indices[1] != 2) FAIL("wrong second index");
...
static int test_parse_commitments_invalid_length(void) {
    ...
    int result = frost_parse_commitments(hex, &session);
    if (result != FROST_CORE_ERR_PARSE) FAIL("invalid length should fail");
...
static int test_parse_commitments_null(void) {
    ...
    if (frost_parse_commitments(NULL, &session) != FROST_CORE_ERR_PARSE) FAIL("NULL hex accepted");
    if (frost_parse_commitments("", NULL) != FROST_CORE_ERR_PARSE) FAIL("NULL session accepted");
...
static int test_parse_sig_share(void) {
    ...
    if (frost_parse_sig_share(valid_hex, out, &len) != 0) FAIL("valid hex failed");
    if (len != 36) FAIL("wrong length");

    char too_long[80];
    memset(too_long, 'a', 74);
    too_long[74] = '\0';
    if (frost_parse_sig_share(too_long, out, &len) != FROST_CORE_ERR_PARSE) FAIL("too long accepted");

    if (frost_parse_sig_share("", out, &len) != FROST_CORE_ERR_PARSE) FAIL("empty accepted");

    if (frost_parse_sig_share(NULL, out, &len) != FROST_CORE_ERR_PARSE) FAIL("NULL hex accepted");
    if (frost_parse_sig_share(valid_hex, NULL, &len) != FROST_CORE_ERR_PARSE) FAIL("NULL out accepted");
    if (frost_parse_sig_share(valid_hex, out, NULL) != FROST_CORE_ERR_PARSE) FAIL("NULL len accepted");
...
static int test_parse_sig_share_short(void) {
    ...
    if (frost_parse_sig_share(short_hex, out, &len) != 0) FAIL("short valid hex failed");
    if (len != 20) FAIL("wrong length for short");
...
static int test_init_signing_session(void) {
    ...
    if (ret != FROST_CORE_OK) FAIL("init returned error");

    if (memcmp(session.session_id, session_id, 32) != 0) FAIL("session_id mismatch");
    if (memcmp(session.message, message, 32) != 0) FAIL("message mismatch");
    if (session.message_len != 32) FAIL("wrong message_len");
    if (session.threshold != 3) FAIL("wrong threshold");
    if (session.participants[0] != 5) FAIL("wrong participant");
    if (session.participant_count != 1) FAIL("wrong participant_count");
    if (session.state != SESSION_AWAITING_COMMITMENTS) FAIL("wrong state");
...
static int test_init_signing_session_null(void) {
    ...
    if (ret != FROST_CORE_ERR_INVALID_SESSION) FAIL("NULL session not rejected");

    ret = frost_init_signing_session(&session, NULL, message, 1, 2);
    if (ret != FROST_CORE_ERR_INVALID_SESSION) FAIL("NULL session_id not rejected");

    ret = frost_init_signing_session(&session, session_id, NULL, 1, 2);
    if (ret != FROST_CORE_ERR_INVALID_SESSION) FAIL("NULL message not rejected");
...
static int test_parse_commitments_overflow(void) {
    ...
    int result = frost_parse_commitments(hex, &session);
    if (result != FROST_CORE_ERR_OVERFLOW) FAIL("overflow not detected");
...
static int test_parse_commitments_malformed(void) {
    ...
    int result = frost_parse_commitments(hex, &session);
    if (result != FROST_CORE_ERR_PARSE) FAIL("malformed commitment should return error");
    if (session.commitment_count != 0) FAIL("count should remain 0");
